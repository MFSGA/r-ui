...
type ImportMode = 'append' | 'replaceByTag' | 'replaceFirstVless';

interface ParsedLine {
  raw: string;
  share: VlessShare | null;
  error: string | null;
}
...
interface VlessBatchImportDialogProps {
  open: boolean;
  onClose: () => void;
  config: XrayConfig;
...
export default function VlessBatchImportDialog({
  open,
  onClose,
  config,
  onConfigUpdate,
  onError,
}: VlessBatchImportDialogProps) {
  const { t } = useI18n();
  ...
  const handleParse = () => {
    ...
    for (const raw of lines) {
      try {
        const share = parseVlessShareLink(raw);
  ...
  return (
    <Dialog open={open} onClose={handleCancel} maxWidth="md" fullWidth>
      <DialogTitle>{t('app.vless.batchTitle')}</DialogTitle>
      <DialogContent dividers>
        <Stack spacing={3} sx={{ pt: 1 }}>
          <Typography variant="body2" color="text.secondary">
            {t('app.vless.batchHint')}
          </Typography>
          ...
          <TextField
            ...
            multiline
            minRows={6}
            label={t('app.vless.batchLabel')}
            placeholder={t('app.vless.batchPlaceholder')}
            value={inputText}
            onChange={(event) => {
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              {isParsing ? t('app.vless.batchParsing') : t('app.vless.batchParse')}
            </Button>
            {isParsing && (
              <Stack spacing={0.5} sx={{ flex: 1 }}>
                ...
                <Typography variant="caption" color="text.secondary">
                  {t('app.vless.parsingProgress', { 1: parseProgress.current, 2: parseProgress.total })}
                </Typography>
              </Stack>
            )}
          </Stack>
          ...
          {hasParsed && (
            <Stack spacing={1.5} ref={resultsContainerRef}>
              ...
              {parsedLines.map((line, index) => {
                ...
                const lineContent = (
                  <Stack
                    ...
                    sx={{
                      p: 1.5,
                      borderRadius: 2,
                      bgcolor: line.share ? 'success.50' : 'error.50',
                      border: (theme) =>
                  ...
                  >
                    {line.share ? (
                      ...
                      <Tooltip title={line.error ?? t('app.vless.invalid')} arrow placement="top">
                        <Badge
                          color="error"
                          badgeContent={'\u2717'}
                    ...
                    <Stack spacing={0.5} sx={{ flex: 1, minWidth: 0 }}>
                      ...
                      {line.share ? (
                        <Stack direction="row" spacing={2} flexWrap="wrap">
                          <Typography variant="caption" color="text.secondary">
                            {t('app.vless.valid')}: {line.share.name ?? line.share.address}:{line.share.port}
                          </Typography>
                          <Typography variant="caption" color="text.secondary">
                            {line.share.params.type} / {line.share.params.security}
                          </Typography>
                        </Stack>
                      ) : (
                ...
                if (!line.share) {
                  return (
                    <Tooltip key={index} title={line.error ?? t('app.vless.invalid')} arrow placement="top">
                      {lineContent}
                    </Tooltip>
                  );
                }
              ...
              <Stack direction="row" spacing={2} alignItems="center">
                <Typography variant="body2" color="success.main" fontWeight={500}>
                  {t('app.vless.batchValidCount', { 1: validCount })}
                </Typography>
                <Typography variant="body2" color="error.main" fontWeight={500}>
                  {t('app.vless.batchInvalidCount', { 1: invalidCount })}
                </Typography>
                {isParsing && (
          ...
          {hasParsed && hasValidLinks && (
            <FormControl fullWidth>
              <InputLabel id="batch-import-mode-label">{t('app.vless.importMode')}</InputLabel>
              <Select
                labelId="batch-import-mode-label"
                label={t('app.vless.importMode')}
                value={importMode}
                onChange={(event) => setImportMode(event.target.value as ImportMode)}
                disabled={isImporting}
              >
                <MenuItem value="append">{t('app.vless.modeAppend')}</MenuItem>
                <MenuItem value="replaceByTag">{t('app.vless.modeReplaceTag')}</MenuItem>
                <MenuItem value="replaceFirstVless">{t('app.vless.modeReplaceFirst')}</MenuItem>
              </Select>
            </FormControl>
          )}
      ...
      <DialogActions>
        ...
        >
          {isImporting ? t('app.vless.batchImporting') : t('app.vless.batchConfirm')}
        </Button>
      </DialogActions>
    </Dialog>
  );
