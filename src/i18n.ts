...
const appTranslations: Record<Locale, Record<string, string>> = {
  'zh-CN': {
    ...
    'app.postUrlHint': '将当前完整配置以 %1 格式作为请求体，通过 POST 发送到目标 URL。目标服务需要允许浏览器跨域请求。',
  ...
  'en-US': {
    ...
    'app.postUrlHint': 'Send the full config as the request body in %1 format with POST. The target service must allow browser CORS requests.',
