...
.PARAMETER XrayPath
    Path to xray.exe. Defaults to the XRAY_PATH environment variable.
...
.EXAMPLE
    .\run-xray-test.ps1 -ConfigPath .\20-hysteria2.json -XrayPath "C:\path\to\xray.exe"
#>
...
# Resolve xray.exe
if (-not $XrayPath) {
    Write-Host "ERROR: xray.exe path was not provided. Set XRAY_PATH or pass -XrayPath." -ForegroundColor Red
    exit 1
}
